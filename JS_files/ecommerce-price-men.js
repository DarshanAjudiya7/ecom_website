$(document).ready(function(){
    ...
    $(".open-content-one-hundred").click(function(){
      ...
      // Show size buttons for this filter
      setTimeout(function() {
        $(".show-two-hundred-page .size-buttons").show();
      }, 100);
    });
    ...
    $(".open-content-two-hundred").click(function(){
      ...
      // Show size buttons for this filter
      setTimeout(function() {
        $(".show-three-hundred-page .size-buttons").show();
      }, 100);
    });
    ...
    $(".open-content-three-hundred").click(function(){
      ...
      // Show size buttons for this filter
      setTimeout(function() {
        $(".show-four-hundred-page .size-buttons").show();
      }, 100);
    });
    //one to four hundred page end//
     ...
     $(".open-content-four-hundred").click(function(){
      ...
      // Show size buttons for this filter
      setTimeout(function() {
        $(".show-five-hundred-page .size-buttons").show();
      }, 100);
    });
    ...
    // Show size buttons for this filter
    setTimeout(function() {
      $(".show-six-hundred-page .size-buttons").show();
    }, 100);
    });
    //one to six hundred page end//
       ...
       $(".open-small-content-one-hundred").click(function(){
        ...
        // Show size buttons for this filter
        setTimeout(function() {
          $(".show-two-hundred-small-page .size-buttons").show();
        }, 100);
      });
      ...
      $(".open-small-content-two-hundred").click(function(){
        ...
        // Show size buttons for this filter
        setTimeout(function() {
          $(".show-three-hundred-small-page .size-buttons").show();
        }, 100);
      });
      //one to three hundred page end//
      ...
      $(".open-small-content-three-hundred").click(function(){
        ...
        // Show size buttons for this filter
        setTimeout(function() {
          $(".show-four-hundred-small-page .size-buttons").show();
        }, 100);
      });
      //one to four hundred page end//
       ...
       $(".open-small-content-four-hundred").click(function(){
        ...
        // Show size buttons for this filter
        setTimeout(function() {
          $(".show-five-hundred-small-page .size-buttons").show();
        }, 100);
      });
      //one to five hundred page end//
      ...
      // Show size buttons for this filter
      setTimeout(function() {
        $(".show-six-hundred-small-page .size-buttons").show();
      }, 100);
      });
      //one to six hundred page end//
...
$('.single-slider').slick({
  ...
  $(document).on('click', '.size-btn', function() {
    // Remove active class from all size buttons in the same product container
    ...
    var productContainer = $(this).closest('.col-xl-3, .col-lg-4, .col-md-4, .col-sm-6');
    
    // Update product display based on size selection
    updateProductDisplay(productContainer, selectedSize);
    
    // Open product detail page in new window
    var productLink = productContainer.find('a[href*="logged_in_for-each-image.html"]').attr('href');
    if (productLink) {
      // Add size parameter to the URL
      var separator = productLink.includes('?') ? '&' : '?';
      var newUrl = productLink + separator + 'size=' + selectedSize;
      window.open(newUrl, '_blank', 'width=1200,height=800,scrollbars=yes,resizable=yes');
    }
  });

  // Function to update product display based on size
  function updateProductDisplay(container, size) {
    var productImage = container.find('img');
    var productLabel = container.find('.image-lable');
    var currentSrc = productImage.attr('src');
    
    // Create size-specific image variations (you can modify this logic based on your needs)
    var sizeSuffix = '';
    switch(size) {
      case 'S':
        sizeSuffix = '-small';
    ...
    // Update image source if size-specific images exist
    var newSrc = currentSrc.replace('.jpg', sizeSuffix + '.jpg');
    productImage.attr('src', newSrc);
    
    // Add visual feedback
    container.addClass('size-selected');
    setTimeout(function() {
      container.removeClass('size-selected');
    }, 500);
    
    // You can add more size-specific logic here
    console.log('Size selected:', size, 'for product:', productLabel.text());
  }
